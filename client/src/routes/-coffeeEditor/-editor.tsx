...
export interface EditorProps {
  id?: string | undefined;
  submitQuery: (input: InferOutput<typeof ModifyCoffeeSchema>) => Promise<TypedResponse<{
    success: boolean,
    id: string
  }>>;
}
...
export function Editor({ id = undefined, submitQuery }: EditorProps) {
  ...
  const { data, isLoading } = useSWR(shouldFetch ? ['coffees', id] : null, rpcFetch(client.api.coffees[':id'].$get)({
    param: {
      id: id ? id : '',
    },
  }));
  ...
  const [state, setState] = useState<'new' | 'loading' | 'success' | 'failure'>(shouldFetch && isLoading ? 'loading' : 'new');
  ...
  const handleSubmit = useCallback(async (e: FormEvent) => {
    ...
    const parsed = safeParse(ModifyCoffeeSchema, { ...values, rating: rating ?? 0 });
    ...
    if (removedImages.length > 0) {
      for (const img of removedImages) {
        const res = await client.api.coffees.images[':id'].$delete({ param: { id: img } });
    ...
    const imageEntries = [...formData.entries()].filter(([k]) => k.startsWith('input-image'));
  ...
  const onImageRemoved = useCallback((img: string) => {
  ...
  return (
    <div className="absolute inset-0 grid-bg">
      ...
      >
        {state === 'success' ? <SuccessMessage /> : <AddShopForm {...shopProps} />}
      </form>
    </div>
  )
    ;
...
function AddShopForm(
  ...
  return (
    <fieldset
      ...
      disabled={loading}>
      <legend className="fieldset-legend">{coffee ? 'Edit Shop' : 'Add Shop'}</legend>

      <div className="flex flex-col gap-1 w-xs">
        <label className="label">Name</label>
        <input name="shopName" className="w-full input" placeholder="Starbuck Coffee"
               defaultValue={coffee?.name ?? ''} />

        <label className="mt-2 label">Lat Long</label>
        <input name="latLong" className="w-full input" placeholder="-7.55, 110.76"
               defaultValue={latLongValue} />

        <label className="mt-2 label">Open Hours</label>
        <input name="openHours" className="w-full input" placeholder="08:00-17:00"
        ...
        <div className="rating rating-half">
          {ratingStars.map(({ v, half }) => (
            <input
              ...
              className={`mask ${half ? 'mask-half-1' : 'mask-half-2'} mask-star-2 bg-accent`}
              aria-label={`${v} star`}
              checked={rating === v}
            />
          ))}
        ...
        <textarea name="address" className="w-full textarea" placeholder="George St. 17"
      ...
      <div className="flex flex-col gap-2 w-xs md:pt-4">
        {
          existingImages.length > 0 && (
            <div className={`flex flex-row gap-2 overflow-x-scroll px-2 pb-2 mx-2 md:pt-2`}>
              {existingImages?.map((image, index) => (
                <div
                  ...
                  <button
                    className={`${index === hovered ? 'block' : 'hidden'} top-0 left-0 right-0 bottom-0 bg-base-content/60 dark:bg-black/80 text-base-100 dark:text-base-content absolute rounded-box flex`}
                    onClick={(e) => {
                      e.preventDefault();
                  ...
                  <img src={image.url} alt={image.alt} className="h-[80px] w-[80px] object-cover rounded-box" />
                </div>
              ))}
            </div>
          )
...
function SuccessMessage() {
  return (
    <div className="m-auto flex flex-col gap-4 p-6 shadow-2xl card bg-base-100">
      ...
      <a target="_blank" href="https://mwyndham.dev/dashboard/articles/new"
         className="btn btn-secondary btn-outline">Create Review Article</a>
    </div>
  );
}
