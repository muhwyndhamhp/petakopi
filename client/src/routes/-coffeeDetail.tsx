...
import { RiCloseLine, RiTimerFlashLine, RiUserLocationLine } from 'react-icons/ri';
...
export const CoffeeDetail = memo(function CoffeeDetail({ coffee }: CoffeeDetailProps) {
  ...
  return (
    <div className="flex w-full drop-shadow-2xl h-[55vh] card bg-base-100 lg:max-h-[94vh] lg:h-fit">
          ...
          {images && images.length > 0 ?
            images?.map((i) =>
              <div className={`carousel-item w-full`}>
                <img className={`w-full object-cover`} src={i.url} alt={i.alt} />
              </div>,
            ) : <img alt="placeholder image" className={`w-full object-cover`}
      ...
      <div className="flex min-h-0 p-0 pb-3 card-body">
        ...
        <div
          className="flex flex-row items-center overflow-ellipsis px-3 py-2 font-light line-clamp-2 hover:bg-base-200">
          <RiTimerFlashLine className={`mr-4`} size={20} />
          <p className={`flex-1/2`}>{`${coffee.open_hours}`}</p>
        </div>
        <div
          className="flex flex-row items-center overflow-ellipsis px-3 font-light min-h-8 hover:bg-base-200">
        ...
        {withReview && (
          <div className="hidden overflow-y-scroll px-4 lg:block">
            <div
              ...
              dangerouslySetInnerHTML={{ __html: review?.post?.encoded_content ?? '' }} />
          </div>
        )}
        {withReview ? (
            <div className="mt-auto flex flex-row lg:mt-0">
              <a className={`btn btn-primary mx-3 mt-3 flex-grow`}
                 target="_blank"
              ...
              {
                access && access !== '' ?
                  <a className={`btn btn-accent btn-outline mr-3 mt-3`}
                     href={`/${coffee.id}/edit`}>
                    Edit
                  </a> :
                  <a className={`btn btn-accent btn-outline mr-3 mt-3`}
          ...
          access && access !== '' ?
            <a className={`btn btn-primary mx-3 mt-auto lg:mt-3`}
               href={`/${coffee.id}/edit`}>
              Edit
            </a> :
            <a className={`btn btn-primary mx-3 mt-auto lg:mt-3`}
